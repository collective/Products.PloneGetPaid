...
from getpaid.wizard import Wizard, ListViewController, \
        interfaces as wizard_interfaces
...
class BaseCheckoutForm(BaseFormView):
    ...
    interface.implements(
        wizard_interfaces.IWizardFormStep,
...
class CheckoutWizard(Wizard):
    """
    a bidirectional checkout wizard, using getpaid.wizard. See the
    CheckoutController class if you want to customize the sequence of steps.
    The Wizard itself is mostly a coordination gatekeeper, that enforces
    checkout constraints transparently to any individual step.

    steps should export all form variables not from themselves as hidden
    inputs, to allow for prepopulated forms from previous inputs on that step.

    steps are specified in order and by view name. a step view must minimally
    have a an update/render cycle ala content_provider or formlib views.

    forms are processed as normal, we depend on a _next_url attribute being
    present and set to the either a url or one of the marker values for next or
    previous steps.

    the form must specify handlers for next / previous / finished steps.

    steps can't override call methods, as such logic won't be processed, since
    we call update / render methods directly.
    """
    ...
    def checkOrderId(self):
        ...
        # if the order id is already in use check to see if its the same user
        # that owns it
        if order_id in order_manager:
            # check for already existing order belonging to the same user from
            # the last day, and redirect them to it, else restart them in the
            # wizard all anonymous get restarted.
            user_id = getSecurityManager().getUser().getId()
            if user_id != 'Anonymous':
                results = order_manager.query(
                    user_id=user_id,
                    creation_date=timedelta(1))
                results = [x for x in results if x.order_id == order_id]
                if len(results) == 1:
                    base_url = self.context.absolute_url()
                    url = base_url + '/@@getpaid-thank-you?order_id=%s' % (
                        order_id)
    ...
    def __call__(self):
        ...
        # check to make sure we have a valid unused order id associated
        # throughout the checkout process. also stores the order id in the data
        # mangaer for use by other components.
...
class CheckoutController(ListViewController):
    ...
    conditions = {
        'checkout-select-shipping': 'checkShippableCart'
    }
    steps = [
...
class CheckoutAddress(BaseCheckoutForm):
    ...
    def hasAddressBookEntries(self):
        ...
        """
        book = component.getUtility(
            IAddressBookUtility).get(getSecurityManager().getUser().getId())
...
class CheckoutReviewAndPay(BaseCheckoutForm):
    ...
    columns = [
        column.GetterColumn(
            title=_(u"Quantity"),
            getter=cart_core.LineItemColumn("quantity")
        ),
        column.GetterColumn(
            title=_(u"Name"),
            getter=cart_core.lineItemURL,
            cell_formatter=formatLinkCell
        ),
        column.GetterColumn(
            title=_(u"Price"),
            getter=cart_core.lineItemPrice
        ),
        column.GetterColumn(
            title=_(u"Total"),
            getter=cart_core.lineItemTotal
        ),
       ]
    ...
    def getSchemaAdapters(self):
        ...
        adapters[formSchemas.getInterface('payment')] = \
                formSchemas.getBagClass('payment')(self.context)
        return adapters
    ...
    def makePayment(self, action, data):
        ...
        order.name_on_card = adapters[
            formSchemas.getInterface('payment')
        ].name_on_card
        order.bill_phone_number = adapters[
            formSchemas.getInterface('payment')
        ].bill_phone_number
        ...
        if result is interfaces.keys.results_async:
            # shouldn't ever happen, on async processors we're already directed
            # to the third party site on the final checkout step, all
            # interaction with an async processor are based on processor
            ...
            if order_template_entry:
                ...
                if uid != 'Anonymous':
                    ...
                    if order_template_entry not in named_orders_list:
                        named_orders_list[order.order_id] = \
                                order_template_entry
            # kill the cart after we create the order
            component.getUtility(
                interfaces.IShoppingCartUtility).destroy(self.context)
        else:
    ...
    def createOrder(self):
        ...
        shipping_code = self.wizard.data_manager.get(
        ...
        if order.shopping_cart.is_recurring():
            ...
            order.renewal_date = order.creation_date + \
                    timedelta(30 * float(first_item.interval))
    ...
    def getNextURL(self, order):
        ...
        if state in (f_states.CHARGEABLE,
                     ...
                     f_states.REVIEWING,
                     f_states.CHARGED):
            return base_url + '/@@getpaid-thank-you?order_id=%s&finance_state=%s' % (
                order.order_id, state)
...
class CheckoutSelectShipping(BaseCheckoutForm):
    """
    browser view for selecting a shipping option and setting it as the shipping
    total for the order.
    """
...
def getShippingMethod(order, service_code):
    ...
    service = component.getUtility(
        interfaces.IShippingRateService, service_name
    )
...
class AddressBookView(BrowserView):
    ...
    def getEntryNames(self):
        ...
        for entry_name in entry_names:
            ...
            entry_dict[entry_name] = """%s<br />%s%s<br />""" % (
                book[entry_name].ship_first_line,
                second_line,
                book[entry_name].ship_city + \
                " " + book[entry_name].ship_postal_code + \
                " " + book[entry_name].ship_state
            )
    ...
    def getEntryScripts(self):
        ...
        """
        addressBookUsr = component.getUtility(
            IAddressBookUtility).get(getSecurityManager().getUser().getId())
