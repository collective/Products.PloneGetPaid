...
class CheckoutController( ListViewController ):
    ...
    def getStep( self, step_name):
        """ Get the BrowserView object rendering the wizard step
        output and/or managing the form processing.

        Note that data_manager is not available in this point, because
        Wizard._extractDataManager() calls this function itself. Thus,
        all step specific context resolving must be done by poking
        form variables directly.
        ...
        # We have a special override for review and pay screen which
        # is payment processor specific
        if step_name == "checkout-review-pay":

            #
            # Here we need to break the recursion loop:
            # - to resolve active payment method needs
            #   wizard._extractDataManagers() to know what wizard data
            #   we have available
            ...
            if payment_processor_name != None:
                ...
                try:
                    ...
                    # Payment processor is somehow misbehaving/running
                    # unit tests with partial data
...
class CheckoutPaymentMethodSelection( BaseCheckoutForm ):
    ...
    2) Each active payment method will provide its own view to display
       the payment method selection button and related info (name,
       description, logo)

    3) The chosen payment method is stored in wizard's data adapters
       using IPaymentMethodInformation storage slot. This storage slot
       is PaymentMethodInfo instance which supports
       getpaid.options.PropertyBag storage.  This is handled by
       getpaid.wizard which automatically extracts and serializes data
       to request form variables.

    Payment method selection is restricted to choices in
    getpaid.paymentprocessors.vocabulary.
    ...
    def update(self):
        ...
        # The following call will set self.form_result and call
        # self.actions (handle_continue) if the request was HTTP POST
